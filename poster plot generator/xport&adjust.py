...
trans_list = b2tp.InputfileParser('C:/Users/user/Documents/GitHub/transport_inputfile/b2.transport.inputfile_29hh', plot= False)
cod = trans_list['1'].T
coki = trans_list['3'].T
coke = trans_list['4'].T
...
for k in specieslist:
    if log_flag:
        ...
        if k == '1':
            ...
            plt.axvline(x=trans_list[k][0,25], color='black',lw=3)
        if k == '4':
            plt.yscale('log')
            plt.axvline(x=trans_list[k][0,12], color='black',lw=3)
